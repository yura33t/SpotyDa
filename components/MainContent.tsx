    ...
    <div className="p-4 md:p-8 pb-40 md:pb-32 animate-page-enter">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 md:gap-4 mb-10">
        {(recommendations || []).slice(0, 8).map(track => (
          ...
          >
            <div className="w-12 h-12 md:w-20 md:h-20 shrink-0 bg-neutral-800 relative">
              <img 
                src={track.coverUrl} 
                alt={track.title} 
            ...
            <div className="pr-3 opacity-0 group-hover:opacity-100 transition-opacity">
               <div className="w-8 h-8 bg-[#1DB954] rounded-full flex items-center justify-center shadow-lg transform translate-y-0 group-hover:scale-110 transition-transform">
      ...
      {recommendations.length > 0 && renderTrackGrid(recommendations, "Made for you")}
    </div>
  );

  const renderSearch = () => (
    <div className="p-4 md:p-8 pb-40 md:pb-32 animate-page-enter">
      <div className="mb-10 sticky top-0 z-10 py-4 bg-black/5 backdrop-blur-sm -mx-4 px-4 md:-mx-8 md:px-8">
